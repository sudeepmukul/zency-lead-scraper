...
"""
╔══════════════════════════════════════════════════════════════════╗
║        Gigzora v2.2 — Intelligent Business Lead Scraper           ║
║        by Zency Studios                      ║
║                                                                  ║
║  Features:                                                       ║
...
PHONE_RE = re.compile(r'''(?x)
    ...
    (?:0?[6-9]\d{9}|             # mobile numbers
       0?40[\s\-]?\d{3,4}[\s\-]?\d{4}|  # Hyderabad landline-ish
       0\d{2,4}[\s\-]?\d{6,8})              # other Indian landlines
''')
    ...
    def _launch(self):
        try:
            ...
            if 'Executable doesn' in str(e) or 'playwright install' in str(e):
                print('❌ Playwright browser is missing. Run: python -m playwright install chromium')
            raise
...
def dedupe_businesses(rows: list) -> list:
    seen, unique = set(), []
    for r in rows:
        name = re.sub(r'\s+', ' ', (r.get('title') or '').lower()).strip()
        phone = re.sub(r'\D+', '', r.get('phone') or '')
        website = (r.get('website') or '').lower().replace('https://', '').replace('http://', '').rstrip('/')
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        cprint(f'\n✅ Found {len(businesses)} unique businesses', '\033[1;32m')
        ...
        if do_web:
            ...
            for i, biz in enumerate(businesses, 1):
                ...
                if website and not any(d in website for d in SKIP_DOMAINS):
                    ...
                    if fb != NIL:
                        tags.append('👍 FB')
                    detail = '  '.join(tags) or ('📞 Maps phone only' if (biz.get('phone') or NIL) != NIL else '🌐 Website found, no email/social')
                else:
                    detail = ('📞 Maps phone only' if (biz.get('phone') or NIL) != NIL else '🚫 no website to scrape')
